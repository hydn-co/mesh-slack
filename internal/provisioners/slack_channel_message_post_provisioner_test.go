...
func TestSlackChannelMessagePostProvisionerShouldReturnCorrectDiscriminator(t *testing.T) {
	opts := &provisioners.SlackChannelMessagePostProvisionerOptions{}
...
func TestSlackChannelMessagePostProvisionerInitShouldSucceed(t *testing.T) {
	ctx := connector.NewTypedFeatureContext[*provisioners.SlackChannelMessagePostProvisionerOptions](
		connector.NewFeatureContext(),
	)
	feature := provisioners.NewSlackChannelMessagePostProvisioner(ctx)
